<?php
  ...
  function map_additional_options($product) {
    ...
    $options[1]->name = 'uv';
    $options[2]->name = 'primer';
    $options[3]->name = 'engrave';

    $options[0]->icon = '-full';
    $options[1]->icon = '-lines';
    $options[2]->icon = '-full-dotted';
    $options[3]->icon = '';

    $options[0]->checkbox = ($product->white_underprint) ? 'empty' : 'unchecked';
    $options[1]->checkbox = ($product->uv_varnish) ? 'empty' : 'unchecked';
    $options[2]->checkbox = ($product->primer) ? 'empty' : 'unchecked';
    $options[3]->checkbox = ($product->engrave) ? 'empty' : 'unchecked';
    ...
    $options[1]->info = ($language == 'HR') ? 'UV lak je prozirna tvrda zaštitna površina ili film. Primjenjuje se kao posljednji korak u postizanju sjajnog ili mat efekta te zaštite. Proizvodima daje dodatnu vrijednost i dubinu.' :
    ...
    $options[2]->info = ($language == 'HR') ? 'Prajmer omogućuje završnu boju da se lijepi mnogo bolje nego da se koristi sama. U tu svrhu temeljni premaz dizajniran je tako da prianja na površine i formira vezni sloj koji je bolje pripremljen za primanje boje.' :
    ...
    $options[3]->info = ($language == 'HR') ? 'Graviranje je proces obrade rotirajućim rezačima za uklanjanje materijala pomicanjem rezača u objekt.' :
